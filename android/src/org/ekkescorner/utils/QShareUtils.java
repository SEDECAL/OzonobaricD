...
public class QShareUtils
{
    ...
    public static boolean viewFile(String filePath, String title, String mimeType, int requestId) {
        ...
        // from a fileUrl:
        // /data/user/0/org.ekkescorner.examples.sharex/files/sedecal_share_files/qt-logo.png



//          uri = Uri.parse((uri.toString() + (char)requestId + "page=5")); // rsm test
//          uri = Uri.parse((uri.toString() + "#p23")); // rsm test
//        uri = Uri.parse((uri.toString() + "?")); // rsm test
//        uri = Uri.parse((uri.toString() + "#" + (char)requestId)); // rsm test
//        uri = Uri.parse((uri.toString() + "?page=13")); // rsm test
//        uri = Uri.parse((uri.toString() + "?pagenumber=13")); // rsm test
//        uri = Uri.parse((uri.toString() + "?defaultPage=13")); // rsm test /view?t=10m6s
//        uri = Uri.parse((uri.toString() + "/view?t=10m6s")); // rsm test
        // uri = Uri.parse((uri.toString() + "&page=5")); // rsm test
        // uri = Uri.parse((uri.toString() + "-page=5")); // rsm test
        // uri = Uri.parse((uri.toString() + ":0.page.25")); // rsm test
        // uri = Uri.parse((uri.toString() + "/image? pagenumber = 13")); // rsm test
        // uri = Uri.parse((uri.toString() + " image? pagenumber=13")); // rsm test
        // uri = Uri.parse((uri.toString() + "/image?pagenumber=13000&w=80000")); // rsm test
        // uri = Uri.parse((uri.toString() + "#pagemode=bookmarks&page=2 ")); // rsm test
//        uri = Uri.parse((uri.toString() + "#pagemode=bookmarks")); // rsm test
        //uri = Uri.parse((uri.toString() + "#features")); // rsm test
//TODO
         uri = Uri.parse((uri.toString() + "#TestPoint")); // rsm test go to concrete place on html file WORKS

//      uri = Uri.parse("file://my_shared_files/BLE_RN4870_53406949_Guide.pdf");// rsm test

//      viewIntent.putExtra("page", 11); //rsm test
//      viewIntent.putExtra("defaultPage", 11); //rsm test
//      viewIntent.putExtra("pageNumber", 11); //rsm test
//      viewIntent.putExtra("pagenumber", 11); //rsm test
//        viewIntent.putExtra(Intent.EXTRA_PROCESS_TEXT, "#page=12");
