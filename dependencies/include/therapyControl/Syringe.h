...
typedef struct syringe_ctx_t
{
  ...
  uint32 *SyringePattern;               /*!< Pointer to and array of limits to determine syringe type (MUST finish with 0 value element). */
}SYRINGE_CTX_T;

void  SYRINGE_InitContext(SYRINGE_CTX_T *Ctx, uint32 *BaseTime, uint32 *SyringePattern);
